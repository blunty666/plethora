...
public class HTMLWriter implements IDocWriter {
	...
	private void writeMethod(MethodData data) {
		...
		writer.printf("<h4 id=\"%s\" class=\"doc-name\">%s</h4>\n", uniqueIdent(data.value), data.name + empty(data.args));
	...
	/**
	 * Attempt to some unique identifier
	 *
	 * @param object The object to convert
	 * @return The unique identifier
	 */
	private String uniqueIdent(Object object) {
		String name = ident(object.getClass().getName());
		...
		for (int id = 0; ; id++) {
			...
			if (existing == object) return fullName;

			if (existing == null) {
				escapeMap.put(fullName, object);
				return fullName;
			}
		}
	}
